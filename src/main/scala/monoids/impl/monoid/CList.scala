...
object CList {
  ...
  def concat[A, B >: A](ms: List[CList[B]]): CList[B] = CList().mconcat(ms)
}

sealed trait CList[+A] extends Monoid[A] {

  // --- Monoid functions

  override def mempty[B >: A]: CList[Nothing] = Nil

  override def mappend[B >: A](that: Monoid[B]): CList[B] = this.append(that.asInstanceOf[CList[B]])

  // alias for mappend overrides super with a more specific return type
  override def <>[B >: A](that: Monoid[B]): CList[B] = this mappend that // alias for mappend overrides super with a more specific return type

  // overrides super method, but gives a more specific return type than Monoid
  override def mconcat[B >: A](ms: List[Monoid[B]]): CList[B] = super.mconcat(ms).asInstanceOf[CList[B]]
