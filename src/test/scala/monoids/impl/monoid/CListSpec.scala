...
class CListSpec extends FlatSpec with Matchers with PropertyChecks {
  ...
  // <> - alias for mappend
  // append - appends two CLists (the same thing with different types)
  // mconcat - concatenates a list of Monoids
  // concat - concatenates a list of CLists (the same thing with different types)

  // you can access the encapsulated values only if you have a CList, Monoid is not sufficient.

  // --- Test CList as a Monoid instance

  "CList() <> CList()" should "be equal to CList()" in {
    CList() mappend CList() shouldEqual CList()
    CList() <> CList() shouldEqual CList()
  }

  "left identity: CList() <> CList(5)" should "be equal to CList(5)" in {
    CList() mappend CList(5) shouldEqual CList(5)
    CList() <> CList(5) shouldEqual CList(5)
  }

  "PROPERTY: left identity" should "hold for Monoid CList[Int]" in {
    forAll { (clist: CList[Int]) =>
      CList() mappend clist shouldEqual clist
      CList() <> clist shouldEqual clist
    }
  }

  "right identity: CList(5) <> CList()" should "be equal to CList(5)" in {
    CList(5) mappend CList() shouldEqual CList(5)
    CList(5) <> CList() shouldEqual CList(5)
  }

  "PROPERTY: right identity" should "hold for Monoid CList[Int]" in {
    forAll { (clist: CList[Int]) =>
      clist mappend CList() shouldEqual clist
      clist <> CList() shouldEqual clist
    }
  ...
  "associativity: (CList(3) <> CList(5)) <> CList(10)" should "CList(3) <> (CList(5) <> CList(10))" in {
    (CList(3) mappend CList(5)) mappend CList(10) shouldEqual (CList(3) mappend (CList(5) mappend CList(10)))
    (CList(3) <> CList(5)) <> CList(10) shouldEqual CList(3) <> (CList(5) <> CList(10))
  }

  "PROPERTY: associativity" should "hold for Monoid CList[Int]" in {
    forAll { (cl1: CList[Int], cl2: CList[Int], cl3: CList[Int]) =>
      (cl1 mappend cl2) mappend cl2 shouldEqual (cl1 mappend (cl2 mappend cl2))
      (cl1 <> cl2) <> cl2 shouldEqual cl1 <> (cl2 <> cl2)
    }
  }

  "mconcat" should "return the CList of a List of CList objects (as a Monoid)" in {
    val clists = List(1, 2, 3, 4, 5).map(CList(_))
    CList().mconcat(clists) shouldEqual CList(1, 2, 3, 4, 5)
  }

  "PROPERTY: mconcat" should "return the CList of a List of CList objects (as a Monoid)" in {
    forAll { (list: List[Int]) =>
      val clists = list.map(CList(_))
      CList().mconcat(clists).asInstanceOf[CList[Int]].toList shouldEqual list
    }
  }

  "concat" should "return the CList of a List of CList objects (as a CList)" in {
    import CList.concat
    val clists = List(1, 2, 3, 4, 5).map(CList(_))
    concat(clists).toList shouldEqual List(1, 2, 3, 4, 5)
  }

  "PROPERTY: concat" should "return the CList of a List of CList objects (as a CList)" in {
    import CList.concat
    forAll { (list: List[Int]) =>
      val clists = list.map(CList(_))
      concat(clists).toList shouldEqual list
    }
  }
  ...
  "CList.append (++, mappend, <>)" should "concatenate two CLists into one" in {
    ...
    CList("Scala", "is") mappend CList("great", "fun") shouldEqual CList("Scala", "is", "great", "fun")
    CList("Scala", "is") <> CList("great", "fun") shouldEqual CList("Scala", "is", "great", "fun")
    CList(1, 2, 3) append CList(4, 5) shouldEqual CList(1, 2, 3, 4, 5)
    CList(1, 2, 3) ++ CList(4, 5) shouldEqual CList(1, 2, 3, 4, 5)
    CList(1, 2, 3) mappend CList(4, 5) shouldEqual CList(1, 2, 3, 4, 5)
    CList(1, 2, 3) <> CList(4, 5) shouldEqual CList(1, 2, 3, 4, 5)
  }

  "CList().mconcat" should "concatenate a List of CLists into one CList" in {
    val listOfCLists = List(CList(1, 2, 3), CList(4, 5), CList(11, 12, 13))
    CList().mconcat(listOfCLists) shouldEqual CList(1, 2, 3, 4, 5, 11, 12, 13) // uses instance method mconcat
    import CList.concat
    CList().mconcat(listOfCLists) shouldEqual CList(1, 2, 3, 4, 5, 11, 12, 13) // uses method mconcat of companion object
    concat(listOfCLists) shouldEqual CList(1, 2, 3, 4, 5, 11, 12, 13) // uses method concat of companion object
    CList().mconcat(List(CList("Scala", "is"), CList("great", "fun"), CList("."))) shouldEqual CList("Scala", "is", "great", "fun", ".")
    concat(List(CList("Scala", "is"), CList("great", "fun"), CList("."))) shouldEqual CList("Scala", "is", "great", "fun", ".")
  }
